...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const sendOrderToWhatsApp = (createdOrder: Order) => {
    ...
    const locationText =
      deliveryMethod === 'pickup'
        ? 'Richmond Hill, Stanley St'
        : `${address}, ${suburb}, ${city}, ${postalCode || '6001'}`;
    ...
    const message = [
      ...
      discountAmount > 0
        ...
        : '',
      `Delivery: ${deliveryFee === 0 ? 'FREE' : formatZAR(deliveryFee)}`,
    ...
    try {
      const orderPayload = {
        ...
        delivery: {
          ...
          address:
            deliveryMethod === 'pickup'
              ? 'Collection at Richmond Hill Bakery Kitchen (Stanley St)'
              : address.trim(),
          suburb:
            deliveryMethod === 'pickup'
              ? 'Richmond Hill'
      ...
      // 1. Save the order to Supabase first.
      const createdOrder = await createOrderData(orderPayload);

      // 2. Open WhatsApp with the order details.
      sendOrderToWhatsApp(createdOrder);

      // 3. Continue with the normal success screen.
        ...
        >
          ...
          <div className="space-y-3 pt-3 border-t border-stone-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              ...
              >
                <div>
                  ...
                  <span className="text-[11px] text-stone-500 block mt-0.5">
                    Stanley St, PE (10am-4pm)
                  </span>
                </div>
                <span className="text-xs font-extrabold text-emerald-600 mt-2 block">
                  FREE (R 0)
                </span>
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-stone-50/80 p-3.5 rounded-2xl border border-stone-200">
                ...
                {deliveryMethod === 'pudo' ? (
                  <div className="sm:col-span-2">
                    ...
                    <span className="text-[10px] text-stone-400 mt-1 block">
                      You will get an SMS PIN directly from PUDO once placed
                      in locker.
                    </span>
                  </div>
                ) : (
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Richmond Hill / Walmer"
                    className="w-full bg-white border border-stone-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-pink-500"
                  />
                </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Ring buzzer 12, or Happy Birthday Thando"
                className="w-full bg-stone-50 border border-stone-200 rounded-xl px-3 py-2 text-xs focus:bg-white focus:outline-none focus:border-pink-500"
              />
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                ...
                },
                {
                  ...
                  desc: 'Richmond Hill only',
                },
              ].map((m) => {
                const isSelected = paymentMethod === m.id;
                const iconClass = `w-4 h-4 ${
                  isSelected ? 'text-pink-600' : 'text-stone-500'
                }`;
            ...
            <div className="bg-pink-50/60 border border-pink-200 p-3.5 rounded-2xl">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-stone-600 mt-0.5">
                      Open your camera or SnapScan app to scan, or click
                      "Place Order & Pay" to simulate instant authorization!
                    </p>
              ...
              {paymentMethod === 'cash' && (
                <p className="text-xs text-stone-700">
                  You can pay cash or swipe card when picking up your box
                  at our Stanley Street kitchen in Richmond Hill between
                  10am and 4pm.
                </p>
              )}
            </div>
          ...
          <div className="pt-3 border-t border-stone-200 space-y-2">
            ...
            <p className="text-center text-[10px] text-stone-400">
              Safe & secure checkout • Freshly baked on dispatch day in
              Mzansi 🇿🇦
            </p>
          </div>
        </form>
